    ...
    const butClearGRBLLog = document.getElementById("butClearGRBLLog");

    const frate = document.getElementById("frate");
    const setFeedRate = document.getElementById("setFeedRate");

    const fbedlength = document.getElementById("fbedlength");
    const setBedLength = document.getElementById("setBedLength");
    ...
    document.addEventListener("DOMContentLoaded", () => {
      ...
      fbedlength.addEventListener("keyup", sendSetBedlengthCommandUponENTER);
      setBedLength.addEventListener("click", sendBedlengthCommand);

      typedCMD.addEventListener("keyup", sendTypedCommandUponENTER);
      sendTypedCMD.addEventListener("click", sendTypedCommand);

      butClearGRBLLog.addEventListener("click", clearGRBLlogArea);
  ...
  function sendUnLockCMD() {
    currCMD = "$X";
    showElaborateCurrCMD("SENDING UNLOCK CMD:", currCMD);
  ...
  function sendHomeCMD() {
    currCMD = "$H";
    showElaborateCurrCMD("SENDING HOMING CMD:", currCMD);
    writeToStream(currCMD);
  }
  ...
  function sendSetFeedRateCommand(){
    ...
    currCMD = setFRcmd;
    showElaborateCurrCMD("SENDING SET FEED-RATE CMD:", currCMD);
    writeToStream(currCMD);
  }
  ...
  function sendBedlengthCommand(){
    var cmd_elems = ['$132=','']
    cmd_elems[1] = fbedlength.value;
    var setBLcmd = cmd_elems.join('');
    // console.log(setFRcmd);
    currCMD = setBLcmd;
    showElaborateCurrCMD("SENDING SET FEED-RATE CMD:", currCMD);
    writeToStream(currCMD);
  ...
  function sendSetBedlengthCommandUponENTER(ev){
    if (ev.keyCode == 13) {
        sendBedlengthCommand();
      }
  }
